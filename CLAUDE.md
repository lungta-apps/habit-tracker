...
client/src/       # React frontend
  components/     # App components
    ...
    MonthHeader.tsx     # Header with month navigation and logout; username area is hidden on mobile (sm:block) and h1 uses text-2xl sm:text-3xl + sm:min-w-64 to avoid exceeding mobile viewport width
    ...
    MonthNoteInput.tsx    # Auto-growing textarea for per-section month notes; Enter blurs/saves, Shift+Enter newline
...
shared/           # Shared between client and server
  schema.ts       # Drizzle schema (users, habits, habit_completions, time_blocks, month_notes) + Zod validation schemas

summaries/        # Session summaries documenting work done per date
```
...
month_notes
  ...
  - userId (varchar, FK -> users.id, cascade delete)
  - month (text, "YYYY-MM")
  - section (text) - "habit" | "project"
  - content (text, default "")
  - updatedAt (timestamp)
  - unique index on (userId, month, section)
```
...
3. **API Routes** (all prefixed `/api`):
   ...
   - Notes: `GET /notes?month=YYYY-MM&section=habit|project`, `PATCH /notes` (upsert)
...
1. **Grid View** (default): Split into two independent sections — **Habits** (top) and **Projects** (bottom) — separated by a divider. Each section has its own scrollable grid, DndContext, add button, and `MonthNoteInput`. The distinction is stored as `itemType` ("habit" | "project") on the habit row. Both sections are identical in behavior: days as columns, rows as items, horizontal scroll, click to toggle completion, double-click to toggle end line, long-press (500ms) for cell value input (pure digits of any length, or 1–2 alpha characters), sticky name column, drag-and-drop reordering. The name column collapse toggle (chevron) is shared across both sections via a single `nameColumnCollapsed` state (persisted in localStorage under key `habit-name-column-collapsed`). Auto-collapses on mobile (window.innerWidth < 640) by default. Reordering within a section sends the full combined ID list (section items first, other section items appended) so the server's `sortOrder` stays coherent across both groups.
   ...
   **Per-week frequency goals**: Habits and projects can have an optional `weeklyTarget` (1–6). When set, a `Nx/wk` chip appears in the name column (inline between the text input and delete button); clicking it opens a popover to change the target. When the name column is collapsed (default on mobile), the chip is replaced by a compact `Nx` label rendered below the color dot in the 32px collapsed cell — this is the only weekly indicator visible without scrolling on mobile. The grid inserts a 20px separator column after each Sunday. In separator cells for habits with a weekly target, a segmented bar (N equal segments with horizontal dividers) fills white from the bottom up as completions accumulate that week — empty segments are dark; the container uses `h-full` so `flex-1` segments always have a defined height. Days in a week where the goal is already met render with a subtle dark tint and a `—` dash instead of the normal empty state. `sundayDays` is computed once in `HabitGrid` (via `useMemo`) and passed to each `HabitRow` so both the header rows and habit rows stay in sync on the same `gridTemplateColumns`.
